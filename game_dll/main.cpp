...
string Get_Body_State(RIGID_BODY<TV> *rd)
{
	...
	QUATERNION<T> ori = rd->Rotation().q;
	ss << "ori:[" << ori.s << "," << ori.v.x << "," << ori.v.y << "," << ori.v.z << "]";
	return ss.str();
}
...
{
	output << "{";
	output << "body:{" << Get_Body_State(rigids_example->jeep) << "}," << endl;
	output << "fltire:{" << Get_Body_State(rigids_example->frontlefttire) << "}," << endl;
	output << "frtire:{" << Get_Body_State(rigids_example->frontrighttire) << "}," << endl;
	output << "bltire:{" << Get_Body_State(rigids_example->backlefttire) << "}," << endl;
	output << "brtire:{" << Get_Body_State(rigids_example->backrighttire) << "}," << endl;
	output << "}";
}
